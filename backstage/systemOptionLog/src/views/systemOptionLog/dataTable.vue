    ...
    >
      ...
      <el-table-column prop="logs" :label="$t('systemOptionLog.actions')">
        <template slot-scope="scope">
          <el-tag
            size="small"
            :type="scope.row.log_type && (scope.row.log_type).indexOf('exception') > -1 ? 'danger' : 'gray'"
          >{{ scope.row.logs | cutWords(50)}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="log_type" :label="$t('systemOptionLog.type')">
        <template slot-scope="scope">
          <span v-if="scope.row.log_type == 'login'">{{$t('systemOptionLog.sysLogin')}}</span>
          <span
            v-if="scope.row.log_type && (scope.row.log_type).indexOf('exception') > -1 "
          >{{$t('systemOptionLog.syserror')}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="createdAt" :label="$t('systemOptionLog.date')"></el-table-column>
      <el-table-column :label="$t('main.dataTableOptions')" width="150">
        <template slot-scope="scope">
          ...
          >
            <svg-icon
              v-show="scope.row.log_type && (scope.row.log_type).indexOf('exception') < 0"
              icon-class="details"
            />
            <svg-icon
              v-show="scope.row.log_type && (scope.row.log_type).indexOf('exception') > -1"
              icon-class="bug"
            />
            <!-- <i
