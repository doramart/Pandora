...
module.exports = (app) => {
  const { DATE, STRING, INTEGER, TEXT, VIRTUAL } = app.Sequelize;
  const Content = app.model.define(
    'content',
    {
      ...
      content_type: {
        type: STRING,
        defaultValue: '1', // 发布类型 1位普通，2为markdown
      },
      ...
      keywords: {
        ...
        get() {
          let targetKeyWords = [];
          const currentKeywords = this.getDataValue('keywords');
          if (currentKeywords) {
            if (currentKeywords.indexOf(',') >= 0) {
              ...
              targetKeyWords.push(currentKeywords);
            }
          }
      ...
      markDownComments: TEXT('long'), // markdown 格式文本
      sImg: STRING(500),
      createdAt: {
        type: DATE,
        get() {
