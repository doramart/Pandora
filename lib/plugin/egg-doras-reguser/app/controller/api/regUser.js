...
let RegUserController = {
    ...
    async getUserInfoById(ctx, app) {
        ...
        try {
            ...
            let targetUser = await ctx.service.user.item(targetId, {
    ...
    async bindEmailOrPhoneNum(ctx, app) {
        ...
        try {
            ...
            if (bindType == '1') {
                ...
                let userRecords = await ctx.service.user.item({
                    query: queryUserObj
                });
    ...
    async loginAction(ctx, app) {
        ...
        try {
            ...
            if (userCount > 0 || loginType == '2' || loginType == '3') {
                ...
                let newUser = await ctx.service.user.item({
                    query: {
                        id: currentUser.id
                    },
                    attributes: getAuthUserFields('login')
    ...
    async reSetPass(ctx, app) {
        ...
        if (keyArr && validator.isEmail(keyArr[1])) {
            ...
            try {
                ...
                let noticeTempPath = '/users/userNotice.html';
                let reSetPwdTempPath = '/users/userResetPsd.html';
    ...
    async addTags(ctx, app) {
        ...
        try {
            ...
            let userInfo = await ctx.service.user.item({
                query: {
    ...
    async followCreator(ctx, app) {
        ...
        try {
            ...
            let targetWatcher = await ctx.service.user.item({
                query: {
                    id: userId
                }
            })
            for (const creatorId of creatorIdArr) {
                ...
                let targetCreatorFollow = await ctx.service.user.item({
                    query: {
                        id: creatorId
                    }
                })
    ...
    async sendVerificationCode(ctx, app) {
        ...
        try {
            ...
            if (messageType == '5') {
                ...
                let targetAdminUser = await ctx.service.adminUser.item({
                    query: {
                        userName,
                        password
                    }
    ...
    async favoriteContent(ctx, app) {
        ...
        try {
            ...
            let favoriteInfo = await ctx.service.userFavoriteContent.item({
                query: {
    ...
    async despiseContent(ctx, app) {
        ...
        try {
            ...
            let userContentAction = await ctx.service.userDespiseContent.item({
                query: {
                    content_id: contentId,
                    user_id: userInfo.id
                }
