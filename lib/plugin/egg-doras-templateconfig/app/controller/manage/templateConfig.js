 ...
 * @Last Modified time: 2021-03-26 16:33:32
 */
'use strict';
...
const TemplateConfigController = {
                    ...
                    if (
                      ...
                      try {
                        ...
                        if (validateTempInfo !== 'success') {
                          ...
                          reject(
                            new Error('extract faild: ' + validateTempInfo)
                          );
                        } else {
                          ...
                          tempObj.using = false;
                          tempObj.items.push(currentItem.id);
  ...
  async uninstallTemp(ctx, app) {
    ...
    try {
      ...
      const defaultTemp = await this._getDefaultTempInfo(ctx, app);
      if (defaultTemp.id === tempId) {
        throw new Error('can not delete using template');
      } else {
        const targetTemp = await ctx.service.contentTemplate.item({
          query: {
            id: tempId,
          },
        });
        ...
        if (!_.isEmpty(targetTemp)) {
          if (!_.isEmpty(targetTemp.items) && targetTemp.items.length > 0) {
            const currentIds = targetTemp.items.map((item) => {
              return item.id;
            });
